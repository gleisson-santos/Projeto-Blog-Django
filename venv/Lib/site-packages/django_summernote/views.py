...
class SummernoteUploadAttachment(UserPassesTestMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        return JsonResponse(
            {"status": "false", "message": _("Only POST method is allowed"),},
            status=400,
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.config["disable_attachment"]:
            ...
            return JsonResponse(
                {"status": "false", "message": _("Attachment module is disabled"),},
                status=403,
            )
        ...
        if not request.FILES.getlist("files"):
            return JsonResponse(
                {"status": "false", "message": _("No files were requested"),},
                status=400,
            )
        ...
        for file in request.FILES.getlist("files"):
            form = UploadForm(files={"file": file,})
            if not form.is_valid():
                ...
                return JsonResponse(
                    {"status": "false", "message": "".join(form.errors["file"]),},
                    status=400,
                )
